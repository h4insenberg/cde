...
export function StatsCard({ 
  ...
  const displayValue = showValue 
    ...
    : '••••';
  return (
    <div className={`rounded-2xl p-3 sm:p-4 ${colorClasses[color]} shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 hover:-translate-y-1`}>
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <p className="text-white/80 text-xs sm:text-sm font-medium">{title}</p>
          <p className="text-base sm:text-2xl font-bold mt-1 leading-tight">
            {displayValue}
          </p>
          {subtitle && (
        ...
        <div className={`p-2 sm:p-3 rounded-xl ${iconBgClasses[color]} flex-shrink-0`}>
