    ...
    <div className="bg-[#2c3e50] rounded-xl p-6 shadow-sm border border-[#3a4a5c]">
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-white flex items-center">
          <Archive className="h-5 w-5 mr-2 text-[#4ade80]" />
          Estoque
        </h3>
      </div>

      {/* Tab Navigation */}
      <div className="flex space-x-1 bg-[#34495e]/50 rounded-lg p-2 mb-4">
        <button
          ...
          className={`flex-1 px-3 py-2 rounded-md transition-colors flex items-center justify-center space-x-2 text-sm font-medium ${
            activeTab === 'alerts'
              ? 'bg-[#3a4a5c] text-[#4ade80] shadow-sm'
            ...
            {lowStockProducts.length > 0 ? (
              <div className="space-y-3">
                {lowStockProducts.map((product) => (
                  <div key={product.id} className="flex items-center justify-between p-3 bg-red-900/20 rounded-lg hover:bg-red-900/30 transition-colors border border-red-800">
