...
export function LowStockAlerts({ products }: LowStockAlertsProps) {
  ...
  return (
    <div className="bg-white/80 dark:bg-[#18191c]/80 backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-200/50 dark:border-gray-700/50">
      ...
      {/* Tab Navigation */}
      <div className="flex space-x-1 bg-gray-100 dark:bg-gray-800/50 rounded-xl p-1 mb-3 sm:mb-4">
        <button
          onClick={() => setActiveTab('alerts')}
          className={`flex-1 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg transition-colors flex items-center justify-center space-x-1 sm:space-x-2 text-xs sm:text-sm font-medium ${
            activeTab === 'alerts'
        ...
        >
          ...
          {lowStockProducts.length > 0 && (
            <span className="bg-red-100 dark:bg-red-950/80 text-red-600 dark:text-red-200 px-1.5 py-0.5 rounded-full text-xs">
        ...
        >
          ...
          <span className="bg-blue-100 dark:bg-blue-950/80 text-blue-600 dark:text-blue-200 px-1.5 py-0.5 rounded-full text-xs">
            ...
            {lowStockProducts.length > 0 ? (
              <div className="space-y-2 sm:space-y-3">
                {lowStockProducts.map((product) => (
                  <div key={product.id} className="flex items-center justify-between p-2.5 sm:p-3 bg-red-50 dark:bg-red-900/20 rounded-xl hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors border border-red-200 dark:border-red-800">
                  ...
                  {recentMovements.filter(m => m.type === 'IN').length > 0 ? (
                    <div className="space-y-1.5 sm:space-y-2">
                      {recentMovements.filter(m => m.type === 'IN').slice(0, 4).map((movement) => (
                        <div key={movement.id} className={`flex items-center justify-between p-2 rounded-xl hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors ${getMovementBg(movement.type)}`}>
                          <div className="flex-1">
                          ...
                          <div className="text-right">
                            <p className={`text-xs sm:text-sm font-semibold ${getMovementColor(movement.type)}`}>
                  ...
                  {recentMovements.filter(m => m.type === 'OUT').length > 0 ? (
                    <div className="space-y-1.5 sm:space-y-2">
                      {recentMovements.filter(m => m.type === 'OUT').slice(0, 4).map((movement) => (
                        <div key={movement.id} className={`flex items-center justify-between p-2 rounded-xl hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors ${getMovementBg(movement.type)}`}>
                          <div className="flex-1">
                            <div className="flex items-center space-x-1.5">
                              {getMovementIcon(movement.type)}
                              <span className="text-xs sm:text-sm font-medium text-gray-900 dark:text-white truncate">
